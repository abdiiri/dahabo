...
const SELECT = "*, vehicles(vehicle_code, plate_number), trips(trip_code)";
...
export async function createFuelRecord(input: NewFuelRecordInput): Promise<FuelRecord> {
  if (isSupabaseConfigured && supabase) {
    ...
    // database (migration 031): its own dense sequence, same scheme as
    // transport orders and trips — never borrowed from a linked trip.
    const { data, error } = await supabase
      .from("fuel_records")
      .insert({
  ...
  // Own dense sequence, exactly like transport orders and trips — never
  // borrowed from a linked trip's number. The linked trip's code (if any)
  // is still captured as tripLabel, purely for display.
  const ref = nextTableRef(store.list().map((f) => f.fuelCode));
  const trip = input.tripId ? await getTrip(input.tripId) : undefined;
...
function mapRow(row: any): FuelRecord {
  return {
    ...
    tripLabel: row.trips?.trip_code ?? undefined,
